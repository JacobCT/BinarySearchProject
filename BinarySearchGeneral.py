...
f = open("pulsarlist.txt","w+")
for k in fromcatoriginal:
	plsr = re.sub("[^0-9]", "", k)
        #Pulsar that you are searching for with RA and DEC only in numbers:
        # plsr = plsrnums   #'0435002749'
	print(plsr)
	...
	b = True
	c = 0
	while (b==True) and (c<len(xarr)):
		xvar = xarr[c]
		# Function call 
		result, directory = binarySearch(pntsrt, l, r, xvar, plsr) 
  
		if result != -1: 
    			print ("Pulsar Detected: " + str(k))
			f.write(str(k) + "	" + str(directory))
			b = False  
